...
final class GemmaE2BProvider: TranscriptionProvider {
    ...
    func modelsExistOnDisk() -> Bool {
        // Cloud API - no local models
        return SettingsStore.shared.getAPIKey(for: "google") != nil
    }

    func clearCache() async throws {
        // No local cache for cloud API
    }

    // MARK: - WAV Encoding

    /// Encodes Float32 PCM samples as a 16-bit PCM WAV file.
    ...
    /// Parses the Gemini API JSON response, extracting transcription text from
...
// MARK: - Data Extension (Little-Endian Helpers)
